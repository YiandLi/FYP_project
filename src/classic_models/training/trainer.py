...
from src.classic_models.training.utils import get_labels, compute_metrics
...
class Trainer(object):
    ...
    def evaluate(self, mode):
        if mode == 'test.sh':
            ...
            raise Exception("Only dev and test.sh dataset available")
        ...
        # 将预测结果写入文件
        if mode == "test.sh":
            f_out = open(os.path.join(self.args.model_dir, "8e-4.csv"), "w", encoding="utf-8")
